...
router.post("/generatebatch", async (req, res) => {
...
router.post("/generatesingle", async (req, res) => {
  // console.log(req.body);
  const { certificateDetails, template_id } = req.body;
  const template = await templateModel.findById(template_id);
  ...
  try {
    ...
    // zip.folder("Minor_Project_Certificates");
    const pdfData = await pdfGenerator.modify(certificateDetails);
    let pdFileName = certificateDetails.studentName.trim().replaceAll(" ", "_")+'.pdf';
    fs.writeFile(`./static/generatedPDF/${pdFileName}`, pdfData, (err) => {
      if (err) throw err;
